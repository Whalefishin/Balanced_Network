...
vector<Edge<V,E,W> > AdjacencyListGraph<V,E,W>::getEdges() {
    ...
    for (int i=0;i<sources.size();i++) {
        ...
        for (int j=0;j<targets.size();j++) {
            ...
            results.push_back(Edge<V,E,W>(
                sources[i], data.first, data.second, targets[j]));
        }
    }
    return results;
}
...
vector<Edge<V,E,W> > AdjacencyListGraph<V,E,W>::getOutgoingEdges(V source) {
    ...
    for (int j=0;j<targets.size();j++) {
        ...
        results.push_back(Edge<V,E,W>(
            source, data.first, data.second, targets[j]));
    }
    return results;
}

template <typename V, typename E, typename W>
vector<Edge<V,E,W> > AdjacencyListGraph<V,E,W>::getIncomingEdges(V destination) {
    vector<Edge<V,E,W> > results;
    vector<V> sources = edges.getKeys();
    for (int i=0;i<sources.size();i++) {
        if (edges.get(sources[i])->contains(destination)) {
            ...
            results.push_back(Edge<V,E,W>(
                sources[i], data.first, data.second, destination));
        }
    }
    return results;
}
